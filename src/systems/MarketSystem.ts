...
export default class MarketSystem {
    cache: Market;
    // have a 15 sec interval that checks every request and if it's filled, remove and tell the creator
    ...
    init() {
        this.interval = setInterval(() => {
            this.cache.sell_requests.forEach((request, requestIndex) => {
            ...
            this.cache.buy_requests.forEach((request, requestIndex) => {
                for(let listingId of Object.keys(request.listings)) {
                    let listing = request.listings[listingId];
                    if(listing.filled) {
                        ClientBot._users.findOne({
